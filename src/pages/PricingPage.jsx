...
import {
  CheckCircle2, Zap, Sprout, ArrowRight,
...
const pricingFaqs = [
  ...
  },
  {
    ...
    answer: "Yes — the Pro plan comes with a 14-day free trial. No credit card is required to start. You'll only be charged after the trial ends if you choose to continue.",
  },
  {
      ...
      <section className="relative pt-28 sm:pt-36 pb-16 overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-base sm:text-lg text-muted-foreground max-w-2xl mx-auto mb-10">
            ...
            Pro plan includes a 14-day trial — no credit card required.
          </p>
          <div className="inline-flex items-center gap-2 bg-muted rounded-xl p-1.5">
            <button
              onClick={() => setIsYearly(false)}
              className={`px-6 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${!isYearly ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-6 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 ${isYearly ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
            >
              Yearly
              <span className="text-xs bg-primary/15 text-primary px-2 py-0.5 rounded-full font-semibold">Save 20%</span>
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="pb-16 sm:pb-24">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-6 lg:gap-8 max-w-3xl mx-auto">
            {plans.map((plan) => {
              ...
              return (
                <Card
                  key={plan.id}
                  className={`relative flex flex-col transition-smooth hover:shadow-elegant ${plan.highlight
                ...
                >
                  ...
                  <CardHeader className="pb-4 pt-8">
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${plan.highlight ? "bg-primary" : "bg-primary/10"
                        }`}>
          ...
          <p className="text-center text-sm text-muted-foreground mt-10">
            Pro plan includes a 14-day free trial. No credit card required. Cancel anytime.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-24">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <Badge className="mb-4 bg-primary/10 text-primary border-primary/20">Compare Plans</Badge>
            <h2 className="text-3xl sm:text-4xl font-display font-bold text-foreground mb-4">
              Everything Side by <span className="text-primary">Side</span>
            </h2>
            <p className="text-muted-foreground">See exactly what you get with each plan before you commit.</p>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-4 px-6 font-semibold text-foreground w-1/2">Feature</th>
                      {plans.map((p) => (
                        <th
                          key={p.id}
                          className={`text-center py-4 px-4 font-display font-semibold ${p.highlight ? "text-primary" : "text-foreground"
                            }`}
                        >
                    ...
                    {comparisonRows.map((row) => (
                      <tr key={row.label} className="hover:bg-muted/30 transition-colors">
                        ...
                        {row.values.map((val, i) => (
                          <td
                            key={i}
                            className={`text-center py-3.5 px-4 ${plans[i].highlight ? "font-medium text-foreground" : "text-foreground"
                              }`}
                          >
                            {val === "✓" ? (
      ...
      <section className="py-16 sm:py-24 bg-muted/30">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <p className="text-muted-foreground">Everything you need to know about plans and payments.</p>
          </div>
          ...
          <Card className="bg-gradient-primary text-primary-foreground border-0 shadow-xl max-w-4xl mx-auto">
            <CardContent className="p-8 sm:p-12 text-center space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center pt-2">
                ...
                <Button size="lg" variant="outline" className="border-white text-white hover:bg-white/10" onClick={() => navigate("/auth?upgrade=pro")}>
                  Upgrade to Pro
                </Button>
              </div>
            </CardContent>
